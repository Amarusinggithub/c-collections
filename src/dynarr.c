...
int append(ArrayPtr arr, const void* value) {
    if (arr == NULL) {
        (void)fprintf(stderr, "[Error] the array is null in%s at line# %d", __FILE__, __LINE__);
        return 1;
    }
    ...
    if (arr->length == arr->capacity) {
        ...
        if (new_arr == NULL) {
            (void)fprintf(stderr, "[Error] the new array is null in%s at line# %d", __FILE__,
                          __LINE__);
            return 1;
        }
        arr->items    = new_arr;
...
int remove_item(ArrayPtr arr, const void* value) {
    ...
    if (new_arr == NULL) {
        (void)fprintf(stderr, "[Error] the new array is null in%s at line# %d", __FILE__, __LINE__);
        return 1;
    }
...
int contains(ArrayPtr arr, const void* value) {
    ...
    if (value == NULL) {
        (void)fprintf(stderr, "[Error] the value is null in%s at line# %d", __FILE__, __LINE__);
        return 1;
    }
...
int insert_at(ArrayPtr arr, const size_t index, const void* value) {
    ...
    for (size_t i = 0; i < arr->length; i++) {
        ...
        memmove((char*)new_arr + (i * arr->element_size),
                (char*)arr->items + (dst_idx * arr->element_size), arr->element_size);
        dst_idx++;
    }
...
int sort(ArrayPtr arr, const Compare cmp) {
    ...
    if (cmp == NULL) {
        (void)fprintf(stderr, "[Error] the cmp function is null in%s at line# %d", __FILE__,
                      __LINE__);
        return 1;
    }
    ...
    if (swap == NULL) {
        (void)fprintf(stderr, "[Error] the swap is null in%s at line# %d", __FILE__, __LINE__);
        return 1;
    }
    for (size_t i = 0; i < arr->length; i++) {
...
int map(ArrayPtr arr, const Function func) {
    ...
    if (func == NULL) {
        (void)fprintf(stderr, "[Error] the fn function is null in%s at line# %d", __FILE__,
                      __LINE__);
        return 1;
    }
    for (size_t i = 0; i < arr->length; i++) {
...
ArrayPtr filter(ArrayPtr arr, const Predicate pred) {
    ...
    if (pred == NULL) {
        (void)fprintf(stderr, "[Error] the predicate function is null in%s at line# %d", __FILE__,
                      __LINE__);
        exit(1);
    }
    ...
    if (new_arr->items == NULL) {
        (void)fprintf(stderr, "[Error] the new array items is null in%s at line# %d", __FILE__,
                      __LINE__);
        exit(1);
    }
    ...
    if (buffer_arr == NULL) {
        (void)fprintf(stderr, "[Error] the buffer array is null in%s at line# %d", __FILE__,
                      __LINE__);
        exit(1);
    }
